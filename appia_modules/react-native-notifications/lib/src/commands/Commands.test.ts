...
describe('Commands', () => {
	...
	beforeEach(() => {
		...
		uut = new Commands(instance(mockedNativeCommandsSender), instance(mockedUniqueIdProvider), notificationFactory);
	});
	...
	describe('getInitialNotification', () => {
		...
		it('Android - returns a promise with the initial notification', async () => {
			...
			when(mockedNativeCommandsSender.getInitialNotification()).thenResolve({ 'google.message_id': 'id' });
		...
		it('iOS - returns a promise with the initial notification', async () => {
			...
			when(mockedNativeCommandsSender.getInitialNotification()).thenResolve({ identifier: 'id' });
			const result = await uut.getInitialNotification();
			expect(result).toEqual(expectedNotification);
		});
	});
	...
	describe('isRegisteredForRemoteNotifications', () => {
		...
		it('return positive response from native', async () => {
			when(mockedNativeCommandsSender.isRegisteredForRemoteNotifications()).thenResolve(true);
		...
		it('return negative response from native', async () => {
			when(mockedNativeCommandsSender.isRegisteredForRemoteNotifications()).thenResolve(false);
			const isRegistered = await uut.isRegisteredForRemoteNotifications();
	...
	describe('checkPermissions', () => {
		...
		it('return negative response from native', async () => {
			...
			when(mockedNativeCommandsSender.checkPermissions()).thenResolve(expectedPermissions);
