...
RCT_EXPORT_METHOD(openPicker:(NSDictionary *)options
    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSInteger maxFiles = abs([[self.options objectForKey:@"maxFiles"] intValue]);
            TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:maxFiles columnNumber:4 delegate:self pushPhotoPickerVc:YES];
            [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto,UIViewController *picker) {
                [weakSelf handlerPicker:picker assets:assets];
            }];
            
            [imagePickerVc setNavLeftBarButtonSettingBlock:^(UIButton *leftButton){
                [leftButton setImage:[UIImage tz_imageNamedFromMyBundle:@"photo_close"] forState:UIControlStateNormal];
                [leftButton setImageEdgeInsets:UIEdgeInsetsMake(0, -10, 0, 20)];
            }];
            
            imagePickerVc.allowTakePicture = NO; // 在内部显示拍照按钮
            imagePickerVc.allowTakeVideo = NO;   // 在内部显示拍视频按
            imagePickerVc.iconThemeColor = [UIColor colorWithRed:40/255.0 green:120/255.0 blue:255/255.0 alpha:1];
            imagePickerVc.oKButtonTitleColorNormal = [UIColor colorWithRed:40/255.0 green:120/255.0 blue:255/255.0 alpha:1];
            imagePickerVc.oKButtonTitleColorDisabled = [UIColor colorWithRed:40/255.0 green:120/255.0 blue:255/255.0 alpha:0.5];
            // 3. 设置是否可以选择视频/图片/原图
            NSString *mediaType = [self.options objectForKey:@"mediaType"];
...
//            imagePickerVc.minImagesCount = abs([[self.options objectForKey:@"minFiles"] intValue]);
            if (maxFiles == 1){
            ...
            imagePickerVc.allowPickingMultipleVideo = [[self.options objectForKey:@"multiple"] boolValue]; // 是否可以多选视频
            
            imagePickerVc.modalPresentationStyle = UIModalPresentationFullScreen;
            [[self getRootVC] presentViewController:imagePickerVc animated:YES completion:nil];
//            // init picker
...
(QBImagePickerController *)imagePickerController
    ...
    [self handlerPicker:imagePickerController assets:assets];
}

- (void)handlerPicker:(UIViewController *)imagePickerController assets:(NSArray *)assets {
    PHImageManager *manager = [PHImageManager defaultManager];
