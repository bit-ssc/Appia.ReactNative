...
- (void)configNavigationBar{
    ...
    containerView.backgroundColor = [UIColor colorWithRed:58/255.0 green:58/255.0 blue:58/255.0 alpha:1];
    containerView.frame = CGRectMake(0, 0, 120, 30);
    ...
    albumNameLabel.frame = CGRectMake(15, 0, 73, 30);
    albumNameLabel.text = self.model.name;
    albumNameLabel.textAlignment = NSTextAlignmentCenter;
    [containerView addSubview:albumNameLabel];
    _albumNameLabel = albumNameLabel;
    
    UIImageView *foldImageView = [[UIImageView alloc] init];
    foldImageView.image = [UIImage tz_imageNamedFromMyBundle:@"photo_album_fold"];
    foldImageView.frame = CGRectMake(88, 4, 22, 22);
    [containerView addSubview:foldImageView];
    _foldImageView = foldImageView;
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeAlbum)];
...
- (void)changeAlbum{
    ...
    if (self->_tableView.isHidden){
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self->_foldImageView.layer.affineTransform = CGAffineTransformMakeRotation(M_PI);
        }];
    }else{
        self->_tableView.hidden = YES;
        [UIView animateWithDuration:0.3 animations:^{
            self->_foldImageView.layer.affineTransform = CGAffineTransformIdentity;
        }];
    }
}
...
- (void)configTableView {
    ...
    if (!_albumArr || _albumArr.count == 0){
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
...
//                                            message:[NSString stringWithFormat:@"%@",self->_models]
...
//
    ...
    self->_tableView.hidden = YES;
    _albumNameLabel.text = model.name;
    [UIView animateWithDuration:0.3 animations:^{
        self->_foldImageView.layer.affineTransform = CGAffineTransformIdentity;
    }];
...
- (void)viewDidLayoutSubviews {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake((self.view.tz_width - fullImageWidth - 56 ) /2, 0, fullImageWidth + 56, 50);
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            if ((model.type == TZAssetCellTypeVideo || model.type == TZAssetCellTypeAudio) && tzImagePickerVc.maxImagesCount == 1){
                NSString *title = @"请选择图片";
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.didFinishPickingPhotosHandle) {
        tzImagePickerVc.didFinishPickingPhotosHandle(photos,assets,_isSelectOriginalPhoto,tzImagePickerVc);
    }
