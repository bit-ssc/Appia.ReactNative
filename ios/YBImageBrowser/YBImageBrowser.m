...
- (void)build {
    ...
    [dismissBtn setFrame:CGRectMake(15, self.frame.size.height - 44, 30, 30)];
    [dismissBtn setImage:[UIImage imageNamed:@"photo_dismiss"] forState:UIControlStateNormal];
    ...
    [downloadBtn setFrame:CGRectMake(self.frame.size.width - 45, self.frame.size.height - 44, 30, 30)];
    [downloadBtn setImage:[UIImage imageNamed:@"photo_download"] forState:UIControlStateNormal];
...
- (void)hide {
  ...
  if ([cell respondsToSelector:@selector(dismissVideo)]) {
    YBIBVideoCell *vCell = cell;
    [vCell dismissVideo];
  }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (pageF < 0) {
      NativeToJSManager *event = [[NativeToJSManager alloc] init];
      [event sendEventWithName:@"slidePhoto" body:@{@"slideDirection":@"previous"}];
      return;
    }
  
    if (pageF > [self.dataMediator numberOfCells] - 1) {
      NativeToJSManager *event = [[NativeToJSManager alloc] init];
      [event sendEventWithName:@"slidePhoto" body:@{@"slideDirection":@"next"}];
      return;
    }
    if (page < 0 || page > [self.dataMediator numberOfCells] - 1) {
      return;
    }
