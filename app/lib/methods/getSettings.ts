...
const loginSettings = [
	// 'API_Gitlab_URL', // 废弃，注册使用
	// 'CAS_enabled', // 废弃，注册使用
	// 'CAS_login_url', // 废弃，单点登录url，注册使用
	// 'Accounts_EmailVerification', // 废弃，注册使用
	// 'Accounts_ManuallyApproveNewUsers', // 废弃，注册使用
	// 'Accounts_ShowFormLogin', // 废弃，注册页面显示登录按钮使用
	// 'Site_Url' // 废弃，旧工作台使用
	// 'Accounts_RegistrationForm', // 废弃，旧工作台使用
	// 'Accounts_RegistrationForm_LinkReplacementText', // 废弃，旧工作台使用
	// 'Accounts_EmailOrUsernamePlaceholder', // 废弃，不使用
	// 'Accounts_PasswordPlaceholder', // 废弃，不使用
	// 'Accounts_PasswordReset', // 废弃，不使用
	// 'Accounts_iframe_enabled', // 废弃，不使用
	// 'Accounts_Iframe_api_url', // 废弃，不使用
	// 'Accounts_Iframe_api_method' // 废弃，不使用
] as string[];
...
export async function getLoginSettings({ server }: { server: string }): Promise<boolean> {
	try {
...
export async function getSettings(): Promise<void> {
	try {
		...
		const result = await fetch(
			...
			.catch(e => {
				console.info('getSettings.public-error', e);
			});
		if (!result?.success) {
			return;
		}
		...
		await db.write(async () => {
			...
			try {
				...
				console.info('getSettings1', e);
				log(e);
			}
			return allRecords.length;
		});
	} catch (e) {
		console.info('getSettings2', e);
		log(e);
	}
}
