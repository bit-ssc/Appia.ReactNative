...
export async function logoutLocal({ server }: { server: string }): Promise<void> {
	// if (roomsSubscription?.stop) {
	// 	roomsSubscription.stop();
	// }
...
function deletePushToken({
	...
	console.info('push.token=start= ', new Date());
	fetch(`${server}/api/v1/push.token`, {
		method: 'DELETE',
		headers: {
	...
	console.info('push.token===end= ', new Date());
}

function lougoutByAPI({ server, authToken, userId }: { server: string; authToken: string; userId: string }) {
...
export async function logoutAfterServerChange({
	server,
	deviceToken,
	authToken,
	userId
	...
	try {
		// UserPreferences.setMap(`${PUSH_TOKEN}-${server}`, { server, deviceToken, authToken, userId });
		await deletePushToken({ server, deviceToken, authToken, userId });
		// if (result?.success) {
		// 	UserPreferences.removeItem(`${PUSH_TOKEN}-${server}`);
		// }
	} catch (e) {
		await deletePushToken({ server, deviceToken, authToken, userId });
		// if (result?.success) {
	...
	try {
		// RC 0.60.0
		await lougoutByAPI({ server, authToken, userId });
	} catch (e) {
		await lougoutByAPI({ server, authToken, userId });
