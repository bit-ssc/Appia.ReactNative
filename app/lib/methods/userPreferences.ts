...
class UserPreferences {
	...
	getRecordTokensArray(): any[] {
		try {
			const result = this.mmkv.getString('recordTokens') ?? null;
			if (result) {
				return result.split(',').map(item => JSON.parse(item));
			}
			return [];
		} catch {
			return [];
	...
	setRecordTokensArray(value: object): boolean | undefined {
		const result = this.getRecordTokensArray();
		result.push(JSON.stringify(value));
		return this.mmkv.setString('recordTokens', result.join(',')) ?? undefined;
	}
