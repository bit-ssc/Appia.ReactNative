...
export default class RoomSubscription {
	...
	updateMessage = (message: IMessage): Promise<void> =>
		new Promise(async resolve => {
			...
			try {
				...
				const recallByMe = message?.u?.username === user?.username;
				if (message.t && message.t.includes('rollback-message') && recallByMe) {
					db.write(async () => {
						const recallMessageCollection = db.get('recall_messages');
						const lastRecords = await recallMessageCollection.query(Q.where('recall_id', null)).fetch();
						if (lastRecords?.length) {
							const lastRecord = lastRecords[0];
							await lastRecord.update(s => {
								s.recallId = message._id;
							});
						}
					});
				}
				...
				} else if (!this.deleteMsgIds.includes(message._id)) {
	...
	handleMessageReceived = (ddpMessage: IDDPMessage) => {
			...
			this.timer = setTimeout(async () => {
				...
				for (let i = 0; i < _queue.length; i += 1) {
					try {
						...
						log(e);
						console.info('handleMessageReceived===error', e);
					}
				}
				...
				try {
					...
					log(e);
					console.info('handleMessageReceived===error2', e);
				}
		...
		if (message.t === 'rollback-message' && message.delMsgId) {
			this.deleteMsgIds.push(message.delMsgId);
			this.deleteMessage(message.delMsgId);
		}
	};

	onPushDeleteMessageId = (delMsgId: string) => {
		this.deleteMsgIds.push(delMsgId);
	};

	deleteMessage = (delMsgId: string) => {
		// console.info('删除失败Server==========start');
		InteractionManager.runAfterInteractions(async () => {
			try {
				const db = database.active;
				const msgCollection = db.get('messages');
				...
				// Delete message
				try {
					const m = await msgCollection.find(delMsgId);
					deleteMessage = m.prepareDestroyPermanently();
				} catch (e) {
					// Do nothing
					// console.info('删除失败Server==========111', e);
				}

				// Delete thread
				try {
					const m = await threadsCollection.find(delMsgId);
					deleteThread = m.prepareDestroyPermanently();
				} catch (e) {
					// Do nothing
					// console.info('删除失败Server==========222', e);
				}

				// Delete thread message
				try {
					const m = await threadMessagesCollection.find(delMsgId);
					deleteThreadMessage = m.prepareDestroyPermanently();
				} catch (e) {
					// Do nothing
					// console.info('删除失败Server==========333', e);
				}
