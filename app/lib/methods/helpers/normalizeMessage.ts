...
import type { IAttachment, ILastMessage, IMessage, IThreadResult } from '../../../definitions';
...
export const normalizeMentions = (msg: ILastMessage | TMsg): ILastMessage | TMsg => {
	if (typeof msg.mentions !== typeof [] || !msg.mentions || !msg.mentions.length) {
		return msg;
	}
	let tmpMsg = msg.msg;
	if (!tmpMsg) {
	...
	msg.mentions.forEach(mention => {
		if (!mention.username || !mention.name) {
			return;
		}
		tmpMsg = tmpMsg === undefined ? '' : tmpMsg;
		tmpMsg = tmpMsg.replace(`@${mention.username}`, `@${mention.name}`);
	});
	msg.msg = tmpMsg;
	return msg;
};
