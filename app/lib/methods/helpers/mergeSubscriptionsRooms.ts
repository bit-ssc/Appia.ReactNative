...
import { store, store as reduxStore } from '../../store/auxStore';
...
import { setUpdateVoiceChatData } from '../../../actions/chat';
...
export const merge = (
	...
	if (room) {
		...
		if (room?.callMsg) {
			const msgData = JSON.parse(room.callMsg) as IVChatCallMsg;
			if (typeof msgData.recordData === 'string') {
				msgData.recordData = JSON.parse(msgData.recordData) as IVCRecordData;
				msgData.recordData.initiatorName = room?.lastMessage?.u?.name;
			}
			if (reduxStore.getState().chat.onCallStatus) {
				if (reduxStore.getState().chat.callMsg?.roomId && reduxStore.getState().chat.callMsg?.roomId === msgData.roomId) {
					store.dispatch(setUpdateVoiceChatData(msgData, room?.onCallStatus));
				}
			} else {
				store.dispatch(setUpdateVoiceChatData(msgData, room?.onCallStatus));
			}
		...
		if (room?._updatedAt) {
			...
			mergedSubscription.callMsg = room.callMsg;
			mergedSubscription.onCallStatus = room.onCallStatus;
			if (room.ts) {
				mergedSubscription.ts = room.ts;
			}
		}
		...
		if (room && 'federated' in room) {
			mergedSubscription.federated = room.federated;
		}
		if (room && 'bot' in room) {
			mergedSubscription.bot = room.bot;
		}
		if (room && 'rt' in room) {
			mergedSubscription.rt = room.rt;
		}
		if (room && 'dname' in room) {
			mergedSubscription.dname = room.dname;
		}
		if (room && 'welcomeMsg' in room) {
			mergedSubscription.welcomeMsg = room.welcomeMsg;
		}
		if (room && 'showAppiaTag' in room) {
			mergedSubscription.showAppiaTag = room.showAppiaTag;
		}
	}
