...
class Sdk {
	...
	handleSwitchUserData(user: any) {
		if (this.current) {
			this.current.client.headers = {
				...this.current.client.headers,
				'X-User-Id': user.id,
				'X-Auth-Token': user.token
			};
			this.current.userId = user.userId;
			this.current.currentLogin = { username: user.username, userId: user.userId, authToken: user.authToken, result: user };
			console.info('this.current.socket', this.current.socket);
			this.current.resume({ token: user.authToken });
		}
	}
