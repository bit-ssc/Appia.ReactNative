...
import { selectServerFailure, serverRequestBySwitchRestore } from '../../actions/server';
...
import { ICredentials, ILoggedUser, RootEnum, STATUSES } from '../../definitions';
...
import { getCompanyToken } from './company';
import { endNetworkTimer } from '../methods/networkTimer';
import { appStart } from '../../actions/app';
import { logInfo } from '../../utils/log';
import { showToast } from '../methods/helpers/showToast';
// import SSLPinning from '../methods/helpers/sslPinning';
...
function connect({ server, logoutOnError = false }: { server: string; logoutOnError: boolean }): Promise<void> {
	return new Promise<void>(resolve => {
		...
		sdk.current
			.connect()
			.then(() => {
				console.info('sdk.current=======connected');
			})
			.catch((err: unknown) => {
				console.info('sdk.current========connect error', err);
			});
...
function loginTOTP(params: ICredentials, loginEmailPassword?: boolean, isFromWebView = false): Promise<ILoggedUser> {
	return new Promise(async (resolve, reject) => {
		try {
			...
			console.info('loginTOTP-error', e);
			if (e.data?.error && (e.data.error === 'totp-required' || e.data.error === 'totp-invalid')) {
				...
				console.info('loginTOTP-error-2', e);
				reject(e);
			}
		}
	});
...
function loginWithPassword({
	...
	userId,
	url
}: ICredentials): Promise<ILoggedUser> {
	...
	} else if (ldap || state.settings.LDAP_Enable) {
		params = {
			username: user,
			ldapPass: password,
			ldap: true,
...
function canRequest() {
	if (sdk.current) {
		return (sdk.current.controller = new AbortController());
	}
}
...
async function getWebsocketInfo({ server }: { server: string }) {
	...
	try {
		// showToast('服务端重连中', 500);

		// await websocketSdk.connect();
		await Promise.race([
			websocketSdk.connect(),
		...
		//	showToast(`服务端连接失败，请关闭APP重试,${err.message}`, 500);
		store.dispatch(switchLogin(false));
...
async function generateSwitchLoginToken(data: any): Promise<{ appiaToken?: string }> {
	store.dispatch(appStart({ root: RootEnum.ROOT_LOADING, text: I18n.t('CompaniesSwitching') }));
	try {
		...
		if (res?.data?.token) {
			return { appiaToken: res.data.token };
		}
		return {};
	} catch (error) {
		...
		console.info('generateSwitchLoginTokenError', error);
		return {};
	}
}

/* async function handleLoginBySwitchRequest(
	otherCompanyUrl: string,
	abortController: AbortController
): Promise<ILoggedUser | undefined> {
	// const certificate = UserPreferences.getString(`${CERTIFICATE_KEY}-${otherCompanyUrl}`);
	// SSLPinning.setCertificate(certificate, otherCompanyUrl);
	...
	try {
		...
		const { username, id, token } = store.getState().login.user;
		const { server } = store.getState().server;

		const data = await fetch(`${otherCompanyUrl}/api/v1/login`, {
			method: 'POST',
			headers: {
		...
		logInfo(server, 'loginBySwitch', 'info', `${endTime - timeStart}`, 'switchServer', id, username);
		...
		requestSwitchCompanyFail();
		return undefined;
	} catch (error) {
		console.info('loginBySwitchError', error);
		requestSwitchCompanyFail();
		return undefined;
	}
} */
...
async function handleLoginBySwitchRequest(
	...
	// const certificate = UserPreferences.getString(`${CERTIFICATE_KEY}-${previousServer}`);
	// if (certificate) {
	// 	SSLPinning?.setCertificate(certificate, previousServer);
	// }
	...
	try {
		...
		connect({ server: nextServer, logoutOnError: false });

		await sdk.current.login({ url: previousServer, userId: id, userToken: token });
		const result = sdk.current.currentLogin?.result;

		const endTime = new Date().getTime();

		logInfo(server, `${previousServer}>${nextServer}`, 'info', `${endTime - timeStart}`, 'switchServer', id, username);
		...
		requestSwitchCompanyFail(previousServer, previousUser);
		return undefined;
	} catch (error) {
		console.info('loginBySwitchError', error);
		requestSwitchCompanyFail(previousServer, previousUser);
...
function requestSwitchCompanyFail(previousServer: string, previousUser: any) {
	showToast(I18n.t('NetworkError'));
	endNetworkTimer();
	if (store.getState().app.root !== RootEnum.ROOT_INSIDE) {
		store.dispatch(appStart({ root: RootEnum.ROOT_INSIDE }));
	}

	store.dispatch(serverRequestBySwitchRestore(previousServer));
	store.dispatch(restorePreviousUser(previousUser));
	store.dispatch(loginSuccessRestore());
}
...
export {
	...
	getLoginServices,
	determineAuthType,
	generateSwitchLoginToken,
	handleLoginBySwitchRequest
};
