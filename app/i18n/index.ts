...
export const getDefaultLanguage = () => {
	const locales = RNLocalize.getLocales && RNLocalize.getLocales();
	if (locales && locales.length > 0) {
		const { languageCode, languageTag } = locales[0];
		if (languageCode && translations[languageCode]) {
			return languageCode;
		}
		if (languageTag && translations[languageTag]) {
			return languageTag;
		}
		if (languageCode === 'zh') {
			return 'zh-CN';
...
export const setLanguage = (l: string) => {
	...
	const locale = LANGUAGES.find(ll => ll.value.toLowerCase() === l.toLowerCase())?.value || 'en';
