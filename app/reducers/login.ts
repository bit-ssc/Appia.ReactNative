...
export const initialState: ILogin = {
	...
	switching: false,
	switchStatus: 0,
	loadingOtherCompanyUserData: false
};
...
export default function login(state = initialState, action: TActionsLogin): ILogin {
	switch (action.type) {
		...
		case types.LOGIN.SWITCH:
			return {
				...state,
				switching: action.payload
			};
		case types.LOGIN.LOGIN_END_BY_SWITCH:
			return {
				...state,
				loadingOtherCompanyUserData: action.loadingOtherCompanyUserData,
				switchStatus: 0
			};
		case types.LOGOUT:
			return initialState;
		case types.LOGIN.LOGIN_SWITCH_STATUS:
			return {
				...state,
