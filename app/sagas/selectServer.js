...
import { clearSettings, addSettings } from '../actions/settings';
import { clearUser, setUser, switchLogin, loginFailure, loginSuccess } from '../actions/login';
...
import { parseSettings, _prepareSettings } from '../lib/methods/parseSettings';
...
const getServerInfo = function* getServerInfo({ server, raiseError = true }) {
	try {
		const serverInfo = { success: true, version: '6.2' };
		// let websocketInfo = { success: true };
		// console.info('getWebsocketInfo=start= ', new Date());
		// if (raiseError) {
		...
		log(e);
		console.info('getServerInfo-error', e);
	}
};
...
const handleSelectServer = function* handleSelectServer({ server, version, fetchVersion }) {
	try {
		...
		console.info('handleSelectServer-error-', e);
		log(e);
	}
};
...
const handleServerRequest = function* handleServerRequest({ server, username, fromServerHistory }) {
	try {
		...
		const certificate = UserPreferences.getString(`${CERTIFICATE_KEY}-${server}`);
		SSLPinning.setCertificate(certificate, server);
		yield getServerInfo({ server });
		const serversDB = database.servers;
		const serversHistoryCollection = serversDB.get('servers_history');
		// if (serverInfo) {
		// yield Services.getLoginServices(server);
		// const res = yield getLoginSettings({ server });

		put(clearSettings());
		// yield put(addSettings({}));
		...
		yield serversDB.action(async () => {
			try {
				...
				log(e);
				put(switchLogin(false));
			}
		});

		yield put(selectServerRequest(server, '6.2', false));
	} catch (e) {
		yield put(serverFailure());
		yield put(switchLogin(false));
		log(e);
...
const handleSelectServerBySwitch = function* handleSelectServerBySwitch({ server }) {
	try {
		// SSL Pinning - Read certificate alias and set it to be used by network requests
		yield put(appStart({ root: RootEnum.ROOT_LOADING, text: I18n.t('CompaniesSwitching') }));
		const certificate = UserPreferences.getString(`${CERTIFICATE_KEY}-${server}`);
		SSLPinning.setCertificate(certificate, server);
...
const handleSelectServerRestore = function* handleSelectServerRestore({ server }) {
	try {
		// SSL Pinning - Read certificate alias and set it to be used by network requests
		...
		EventEmitter.emit('getHomepageList');
	} catch (e) {
		endNetworkTimer();
		showToast(I18n.t('NetworkError'));
		yield put(serverFailure());
...
const root = function* root() {
	...
	yield takeLatest(SERVER.REQUESTBYSWITCH, handleSelectServerBySwitch);
	yield takeLatest(SERVER.REQUESTBYSWITCH_RESTORE, handleSelectServerRestore);
};
export default root;
