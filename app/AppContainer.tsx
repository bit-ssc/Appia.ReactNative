...
const App = memo(
	({ root, isMasterDetail, server, user }: { root: string; isMasterDetail: boolean; server: string; user: any }) => {
		...
		return (
			<NavigationContainer
				...
				onStateChange={state => {
					...
					TalkingDataSDK.onPageEnd(`${previousRouteName}`);
					TalkingDataSDK.onPageBegin(currentRouteName);
					Navigation.routeNameRef.current = currentRouteName;
				}}
			>
