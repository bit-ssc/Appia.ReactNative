...
import { IApplicationState, IAttachment, IEmoji, TAnyMessageModel } from '../../definitions';
...
class MessageContainer extends React.Component<IMessageContainerProps, IMessageContainerState> {
	...
	shouldComponentUpdate(nextProps: IMessageContainerProps, nextState: IMessageContainerState) {
		...
		const { threadBadgeColor, isIgnored, highlighted, previousItem, fastModeMessage } = this.props;
		...
		if (nextProps.fastModeMessage !== fastModeMessage) {
			return true;
		}
		if (nextProps.item.attachments !== this.props.item.attachments) {
			return true;
		}
		return false;
	}
	...
	onPressAction = () => {
		 ...
		 */
		const { messageMultiSelect, item, onPress, jumpToMessage } = this.props;

		// 判断为多选 onPress函数存在  msg_type变量存在且不为通话类型
		if (messageMultiSelect && onPress && 'msg_type' in item._raw && item._raw.msg_type !== 'oncall') {
			onPress(item);
			return;
		}
		if (jumpToMessage) {
	...
	onLongPress = () => {
		...
		if (this.isInfo || this.hasError || this.isEncrypted || archived || isUploadFile) {
			return;
		}
		if (onLongPress) {
			onLongPress(item);
	...
	render() {
		...
		let message = fastModeMessage || msg;
