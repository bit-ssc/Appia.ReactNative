...
export function serverRequestBySwitch(server: string): IServer {
	return {
		type: SERVER.REQUESTBYSWITCH,
		server,
		username: null,
		fromServerHistory: false
	};
}

export function serverRequestBySwitchRestore(server: string): IServer {
	return {
		type: SERVER.REQUESTBYSWITCH_RESTORE,
		server,
		username: null,
		fromServerHistory: false
	};
