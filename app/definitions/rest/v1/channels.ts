...
import type { IServerRoom, MeetingJump } from '../../IRoom';
import type { IExternalData, IUser } from '../../IUser';
...
export type ChannelsEndpoints = {
	...
	'meeting.next': {
		GET: (params: { roomId: IServerRoom['_id']; username: string }) => {
			data: MeetingJump[];
		};
	};
	'org/members': {
		GET: (params: { rid?: IServerRoom['_id'] }) => {
			data: IExternalData[];
		};
	};
	'otkr.canQuery': {
		POST: (params: { owner: string; viewer: string }) => {
			data: boolean;
			success: boolean;
		};
	};
