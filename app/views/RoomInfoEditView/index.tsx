...
import { getRoomTitle, hasPermission, isAndroid } from '../../lib/methods';
...
class RoomInfoEditView extends React.Component<IRoomInfoEditViewProps, IRoomInfoEditViewState> {
	...
	static navigationOptions = ({ navigation }: IRoomInfoEditViewProps) => ({
	...
	formIsChanged = () => {
		...
		return !(
			...
			room.topic === topic &&
			room.announcement?.message === announcement &&
	...
	submit = async () => {
		...
		if (room.announcement?.message != announcement) {
		...
		Alert.alert(
			...
			[
				...
				},
				{
					...
					onPress: () => {
						// sendLoadingEvent({ visible: true });
						dispatch(deleteRoom(ERoomType.c, room));
					}
				}
			],
			{ cancelable: false }
	...
	render() {
		const { name, nameError, description, room, avatar } = this.state;
