package chat.rocket.reactnative.imagepicker.ui;
...
import chat.rocket.reactnative.imagepicker.DataHolder;
import chat.rocket.reactnative.imagepicker.ImageDataSource;
import chat.rocket.reactnative.imagepicker.ImagePicker;
import chat.rocket.reactnative.imagepicker.adapter.ImageFolderAdapter;
import chat.rocket.reactnative.imagepicker.adapter.ImageRecyclerAdapter;
...
import chat.rocket.reactnative.imagepicker.view.FolderPopUpWindow;
import chat.rocket.reactnative.imagepicker.view.GridSpacingItemDecoration;
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_PERMISSION_STORAGE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                showToast("请确认打开存储访问权限");  //权限检测目前存在问题，有权限但是check出来是没有权限的
            }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_ok) {
            ...
            if (mFolderPopupWindow.isShowing()) {
                ...
                mFolderPopupWindow.showAtLocation(mTopBar, Gravity.NO_GRAVITY, 0, 0);
...
//                    mImageGridAdapter.refreshData(imageFolder.images);
    ...
    public void onImagesLoaded(List<ImageFolder> imageFolders) {
        if(mImageFolders == null) {
            ...
            this.mImageFolders = ImagePicker.getInstance().getImageFolders();
...
//            intent.putExtra(ImagePicker.EXTRA_IMAGE_ITEMS, imagePicker.getCurrentImageFolderItems());
            ...
            // 但采用弱引用会导致预览弱引用直接返回空指针 解决预览时仍然在加载图片而引起的崩溃的问题
            DataHolder.getInstance().save(DataHolder.DH_CURRENT_IMAGE_FOLDER_ITEMS, new ArrayList<ImageItem>(imagePicker.getCurrentImageFolderItems()));
            intent.putExtra(ImagePreviewActivity.ISORIGIN, isOrigin);
